...
public class AppiumPlugin extends PluginAbstract implements PluginAbstractReport {
     ...
     */
    public void createWidgets(List<WebElement> elements) {
        textElementNumber = 0;
        ...
        List<Widget> widgets = new ArrayList<>();//StateController.getCurrentState().getAllWidgets();
                ...
                while (!webeles.isEmpty()) {
                    ...
                    if (element != null) {
                        ...
                        if (!existingKeys.contains(key)) {
                            Widget widget = createWidgetFromElement(element, position, desc, text, "true", resourceId, key);
     ...
     * @return Widget
     */
    private Widget createWidgetFromElement(WebElement element, String position, String desc, String text, String clickable, String resourcId, String uniqueId) {
    ...
    private static void handleTyping(Action action, Widget widget) {
        ...
        if(keyCode == KeyEvent.VK_ENTER) {
            ...
            try {
                String id = widget.getMetadata("id").toString();
                if (!id.trim().equalsIgnoreCase("null")) {
